...
Completed on 2026-09-11 in `.`, on `main`.
...
InfoMentor's existing native builder deliberately requires Bun 1.4.2. A private temporary installation at `${TMPDIR}/family-mcp-phase1-runtime/node_modules/.bin/bun` supplied that version for native checks and a separate forced CI-equivalent run. Its frozen install accepted the Bun 1.2.19-generated lockfile without modification. No release version guard was weakened.

The default npm cache was not writable in the sandbox, so package checks used `${TMPDIR}/family-mcp-phase1-npm-cache`. No ownership repair or global package-manager installation was performed. Temporary runtime/cache data and ignored package/native artifacts remain local.
