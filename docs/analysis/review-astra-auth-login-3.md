...
The temporary harness is `${TMPDIR}/astra-auth-login-3-X2w2vs/probes.ts`; each named case has `result.json` and `trace.jsonl` beneath that directory. It can be rerun while the scratch directory remains available:

```sh
bun ${TMPDIR}/astra-auth-login-3-X2w2vs/probes.ts
bun ${TMPDIR}/astra-auth-login-3-X2w2vs/probes.ts --before-ready
bun ${TMPDIR}/astra-auth-login-3-X2w2vs/probes.ts --pid-spoof
```
