...
- Pinned Bun 1.4.2: `PATH=${TMPDIR}/family-mcp-phase1-runtime/node_modules/.bin:$PATH bunx turbo run typecheck lint format:check test --force` — 20/20 Turbo tasks passed; the same 46 tests passed.
- Pinned Bun 1.4.2: `PATH=${TMPDIR}/family-mcp-phase1-runtime/node_modules/.bin:$PATH bunx turbo run test:binary test:installer --force` — 6/6 Turbo tasks passed. Both standalone binaries passed MCP smoke checks (six Abler tools and seven InfoMentor tools); each installer passed 12 piped cases, truncation checks, and archive-install checks.
