...
main() {
  ...
  package_dir="$prefix/share/dominos-mcp"
  current_uid=$(id -u)
  ...
  command_matches_install() {
    case "$1" in
      "$2/bin/dominos-mcp"|"$2/bin/dominos-mcp "*|"$2/bin/dominos-mcp-warp"|"$2/bin/dominos-mcp-warp "*|\
      /bin/sh\ "$2/bin/dominos-mcp"|/bin/sh\ "$2/bin/dominos-mcp "*|\
      /bin/sh\ "$2/bin/dominos-mcp-warp"|/bin/sh\ "$2/bin/dominos-mcp-warp "*|\
      sh\ "$2/bin/dominos-mcp"|sh\ "$2/bin/dominos-mcp "*|\
      sh\ "$2/bin/dominos-mcp-warp"|sh\ "$2/bin/dominos-mcp-warp "*) return 0 ;;
    esac
    return 1
  }
...
"*) return 0 ;;
  ...
  process_matches_install() {
    ...
    if [ "$platform" = linux ]; then
      ...
      case "$executable" in "$2/bin/dominos-mcp"|"$2/bin/dominos-mcp-warp") return 0 ;; esac
  ...
  detect_linux_processes() {
    for process_link in /proc/[0-9]*/exe; do
      ...
      case "$executable" in
        "$package_dir/"*/bin/dominos-mcp|"$package_dir/"*/bin/dominos-mcp-warp)
  ...
  detect_macos_symlink_processes() {
    ...
    ps -axo pid=,command= | while read -r process_pid command; do
      ...
      case "$command" in "$prefix/bin/dominos-mcp"|"$prefix/bin/dominos-mcp "*) ;; *) continue ;; esac
  ...
  temporary=$(mktemp -d "${TMPDIR:-/tmp}/dominos-install.XXXXXX")
  staging=
  command_tmp=
  trap 'rm -rf "$temporary"; if [ -n "$staging" ]; then rm -rf "$staging"; fi; if [ -n "$command_tmp" ]; then rm -rf "$command_tmp"; fi' 0
  trap 'exit 1' 1 2 15
  archive="dominos-mcp-$version-$platform-$arch.tar.gz"
  release="https://github.com/olafurns7/family-mcp/releases/download/dominos-mcp@$version"
  ...
  [ ! -d "$prefix/bin/dominos-mcp" ] || { echo 'Install destination is a directory; nothing was replaced.' >&2; exit 1; }
  ...
  tar -xOzf "$temporary/$archive" 'dominos-mcp/bin/dominos-mcp' > "$staging/bin/dominos-mcp"
  chmod 755 "$staging/bin/dominos-mcp"
  [ "$("$staging/bin/dominos-mcp" --version </dev/null)" = "$version" ] || { echo 'Downloaded executable returned an unexpected version; previous installation kept.' >&2; exit 1; }
  for file in LICENSE README.md THIRD_PARTY_NOTICES.txt; do
    tar -xOzf "$temporary/$archive" "dominos-mcp/$file" > "$staging/$file"
  done

  entrypoint=dominos-mcp

  install_dir="$prefix/share/dominos-mcp/$version-$digest"
  if [ -e "$install_dir" ] || [ -L "$install_dir" ]; then
    [ ! -L "$install_dir" ] && [ -d "$install_dir" ] && [ "$("$install_dir/bin/dominos-mcp" --version </dev/null)" = "$version" ] || { echo 'Existing installation is invalid.' >&2; exit 1; }
    rm -rf "$staging"
  else
  ...
  if [ -L "$prefix/bin/dominos-mcp" ]; then
    previous_target=$(readlink "$prefix/bin/dominos-mcp" 2>/dev/null || true)
  ...
  command_tmp=$(mktemp -d "$prefix/bin/.dominos-mcp.XXXXXX")
  ln -s "$install_dir/bin/$entrypoint" "$command_tmp/command"
  mv -f "$command_tmp/command" "$prefix/bin/dominos-mcp"
  ...
  if [ -s "$old_processes" ]; then
    if [ "$stop_running" = true ]; then
      ...
      while IFS="$(printf '\t')" read -r process_pid candidate; do
        if [ -z "$candidate" ]; then
          printf 'Possibly old dominos-mcp process still running: PID %s (version unknown)\n' "$process_pid"
          remaining=true
        elif process_matches_install "$process_pid" "$candidate"; then
          printf 'Old dominos-mcp process still running after SIGTERM: PID %s (%s)\n' "$process_pid" "$candidate"
          remaining=true
        else
          printf 'Stopped old dominos-mcp process: PID %s (%s)\n' "$process_pid" "$candidate"
        fi
      done < "$old_processes"
      if [ "$remaining" = true ]; then
        printf 'Install succeeded, but old dominos-mcp processes remained after SIGTERM. Restart your MCP host after stopping them.\n' >&2
        return 1
      fi
    else
      while IFS="$(printf '\t')" read -r process_pid candidate; do
        if [ -n "$candidate" ]; then
          printf 'Old dominos-mcp process still running: PID %s (%s)\n' "$process_pid" "$candidate"
        else
          printf 'Possibly old dominos-mcp process still running: PID %s (version unknown)\n' "$process_pid"
        fi
      done < "$old_processes"
  ...
  printf 'Installed dominos-mcp %s at %s/bin/dominos-mcp\n' "$version" "$prefix"
  printf 'Use that absolute path in your MCP client. Setup: https://github.com/olafurns7/family-mcp/tree/dominos-mcp@%s/packages/dominos-mcp#readme\n' "$version"
