...
| Session busy                                     | Requests already wait up to 30 seconds; retry when the active request ends. A crashed process releases its lock immediately; an abandoned lock expires after two minutes. Only if it persists, stop every abler-mcp process and remove the `session.json.lock` directory.                                |
